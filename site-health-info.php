...
if ( ! class_exists( 'WP_Debug_Data' ) ) {
	require_once __DIR__ . '/class-wp-debug-data.php';
}
if ( ! class_exists( 'WP_Site_Health' ) ) {
...
?>
	...
	<div class="wrap health-check-header">
		...
		<nav class="tabs-wrapper" aria-label="<?php esc_attr_e( 'Secondary menu' ); ?>">
			...
			<a href="<?php echo esc_url( shrip_admin_url() ); ?>" class="tab">
				<?php _e( 'Status' ); ?>
			</a>

			<!-- <a href="<?php echo esc_url( admin_url( 'site-health.php?tab=debug' ) ); ?>" class="tab active" aria-current="true"> -->
			<a href="<?php echo esc_url( shrip_admin_url( [ 'tab' => 'debug' ] ) ); ?>" class="tab active" aria-current="true">
				<?php _e( 'Info' ); ?>
			</a>
		</nav>
